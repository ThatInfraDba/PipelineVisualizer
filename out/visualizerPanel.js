...
exports.PipelineVisualizerPanel = void 0;
...
class PipelineVisualizerPanel {
    static createOrShow(extensionUri, yamlContent, pipelineData, layoutPreference, colorTheme, fileName = '') {
        ...
        const panel = vscode.window.createWebviewPanel(PipelineVisualizerPanel.viewType, 'Pipeline Visualization', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    static revive(panel, extensionUri, _state) {
        PipelineVisualizerPanel.currentPanel = new PipelineVisualizerPanel(panel, extensionUri);
    }
    ...
    _update(yamlContent, pipelineData, layoutPreference, colorTheme, fileName = '') {
    ...
    _getHtmlForWebview(webview, pipelineData, layoutPreference, colorTheme, fileName = '') {
...
<body class="${platformClass}">
    ...
    _detectPlatform(data, fileName = '') {
        // Filename-based signals (strongest)
        if (fileName.toLowerCase().includes('gitlab-ci')) {
            return 'gitlab';
        }
        if (fileName.toLowerCase().includes('buildspec')) {
            return 'aws-codebuild';
        }
        if (fileName.toLowerCase().includes('bitbucket-pipelines')) {
            return 'bitbucket';
        }
        ...
        if (data.AWSTemplateFormatVersion) {
            return 'aws-cloudformation';
        }
        if (data.Resources && typeof data.Resources === 'object' &&
            Object.values(data.Resources).some((r) => r && r.Type && String(r.Type).startsWith('AWS::'))) {
            return 'aws-cloudformation';
        }
        // GitHub Actions: 'on' is the definitive marker
        if (data.on || (data.jobs && !Array.isArray(data.jobs) && Object.values(data.jobs).some((j) => j['runs-on'] || j.uses))) {
            return 'github';
        }
        ...
        if (data.pool || (data.jobs && Array.isArray(data.jobs)) || data.trigger || data.pr) {
            return 'azure';
        }
        // GitLab: stages is an array of strings (Azure stages are objects with stage/jobs properties)
        if (Array.isArray(data.stages) && data.stages.some((s) => typeof s === 'string')) {
            return 'gitlab';
        }
...
exports.PipelineVisualizerPanel = PipelineVisualizerPanel;
PipelineVisualizerPanel.viewType = 'pipelineVisualizer';
